...
check('All-range selectors (Owner 9/9: Macros range is the short "All" so 3 day / 7 day / All stay on one line; Workout range keeps "All time")', /<button data-w="0">All</button><button data-avg="1">Average</.test(pageTpl) && /data-w="0">All time</button><button data-w="custom">Custom</.test(pageTpl));
...
if (typeof mod.chartPage !== 'function') { failures++; console.log('  FAIL chartPage export'); }
  ...
  check('render: search UI in built HTML', html.includes('id="msrchbtn"') && html.includes('id="msrchrow"') && html.includes('id="msrchq"') && html.includes('id="msrchres"') && !html.includes('msrchseg'));
